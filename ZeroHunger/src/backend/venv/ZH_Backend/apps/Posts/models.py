...
class RequestPost(models.Model):
    ...
    postedBy = models.ForeignKey(BasicUser, on_delete=models.CASCADE, related_name="request_post_by", default=1)
...
class OfferPost(models.Model):
    ...
    images = models.CharField(max_length=256, blank=True) #link to images in DB
    postedOn = models.IntegerField(default=1) #Stores time as unix timestamp, defaults to UTC
    postedBy = models.ForeignKey(BasicUser, on_delete=models.CASCADE, related_name="offer_post_by", default=1)
    description = models.CharField(max_length=1024, blank=True)
    def __str__(self):
        return self.title
